...
def change ():
    if var.get() == 0:
        b_decrypt.config(command = decrypt_button())
        b_encrypt.config (command = encrypt_button())
    if var.get() == 1:
        entry_the_key2.place(x=470, y=40)
        enter_the_key2.place (x = 470, y = 10)
...
enter_the_key2 = Label(text='Enter your second key:',font="Arial 14")
...
b_change = Button( text = 'Change', width = 10, height = 1, font = 'Arial 14', command = change)
b_change.place (x=10, y=130)
